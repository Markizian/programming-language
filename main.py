class lv:
    precedence = {'+': 1, '-': 1, '*': 2, '/': 2}
    function_arg_count = {'veids': 1, 'zīmēt': 2, 'mainīt': 2} #dictionary  of the number of function arguments
    output = ""
    error = ""
    ...
    def errors_list(self, error_code):
        list_of_errors = {
            ...
            'Sintakses kļūda: Par daudz': 'Sintakses kļūda: Par daudz vērtību',
    ...
    def process_string_in_code(self,code):
        #reworking the code text for convenient processing
        code_ready = ""
        str_detected = False
        line = 1
        last_char = ""
    ...
    def process_string_in_token(self, str):
        #replacement of characters in the code text to be able to convert it into tokens
        str = str.replace("_"," ")
        str = str.replace('"',"")
        return str
    ...
    def stack_collapse(self):
        ...
        #if there is a function at the beginning of a line of code, this will process it
        if args[0][0][0] == "funkcija":
            self.stack_push(self.function_option(args[0][0][1], args[1::]))
            output = self.stack.pop()
        elif len(args) != 1:
            self.raise_error(f'{self.errors_list("Sintakses kļūda: Par daudz")}, uz rindas {self.line_nr}')
        else:
            output = args.pop()[0]
    ...
    def parse_for_loop_statement(self):
        ...
        token = self.next_token()
        counter = 0 #how many times the cycle will repeat
        ...
        if token[0] == 'mainīgais':
            if token[1] not in self.vars:
                ...
                value = self.vars[token[1]]
                if value[0] == "veselnieks":
                    counter = value[1]
                else:
        ...
        token_memory = [('\n',)] #a piece of code is written here and repeated as many times as the loop runs
        token = self.next_token()
        while token[0] != '\n':
            if token[0] == 'vēl':
                token_memory.append(('\n',))
    ...
    def parse_if_statement(self):
        ...
        token = self.next_token()
        identifier = "" #what we're comparing
    ...
    def parse_print_statement(self):
        ...
        if not self.parse_expression() or len(self.stack) == 0:
    ...
    def parse_assignment(self):
        ...
        if token[0] != 'mainīgais':
            self.return_token(token)
            return False
        identifier = token[1] #variable name
...
if __name__ == '__main__':
    f = open("tests.lv", "r", encoding="utf-8") #can be any file with the extension .lv
    program = lv(f.read())
    program.run()
